/** @type {import('tailwindcss').Config} */
module.exports = {
  darkMode: "class",
  content: ["./src/**/*.{js,jsx,ts,tsx}", "./.storybook/**/*.{js,jsx,ts,tsx}"],
  presets: [require("nativewind/preset")],
  theme: {
    borderRadius: {
      none: "0px",
      xxs: "2px",
      xs: "4px",
      sm: "6px",
      DEFAULT: "8px",
      md: "8px",
      lg: "10px",
      xl: "12px",
      "2xl": "16px",
      "3xl": "20px",
      "4xl": "24px",
      full: "9999px",
    },
    extend: {
      spacing: {
        none: "0px",
        xxs: "2px",
        xs: "4px",
        sm: "6px",
        md: "8px",
        lg: "12px",
        xl: "16px",
        "2xl": "20px",
        "3xl": "24px",
        "4xl": "32px",
        "5xl": "40px",
        "6xl": "48px",
        "7xl": "64px",
        "8xl": "80px",
        "9xl": "96px",
        "10xl": "128px",
        "11xl": "160px",
        "container-padding-mobile": "16px",
        "container-padding-desktop": "32px",
      },
      colors: {
        /* ── Primitive color palette ─ usage: bg-base-white, text-brand-500, etc. ── */
        base: {
          white: "#FFFFFF",
          black: "#000000",
          transparent: "transparent",
        },
        "gray-light": {
          25: "#FCFCFD",
          50: "#F8F9FC",
          100: "#EAECF5",
          200: "#D5D9EB",
          300: "#B3B8DB",
          400: "#717BBC",
          500: "#4E5BA6",
          600: "#3E4784",
          700: "#363F72",
          800: "#293056",
          900: "#101323",
          950: "#0D0F1C",
        },
        "gray-dark": {
          25: "#FCFCFD",
          50: "#F9F9FB",
          100: "#EFF1F5",
          200: "#DCDFEA",
          300: "#B9C0D4",
          400: "#7D89B0",
          500: "#5D6B98",
          600: "#4A5578",
          700: "#404968",
          800: "#30374F",
          900: "#111322",
          950: "#0E101B",
        },
        "gray-dark-alpha": {
          25: "rgba(255, 255, 255, 0.98)",
          50: "rgba(255, 255, 255, 0.96)",
          100: "rgba(255, 255, 255, 0.94)",
          200: "rgba(255, 255, 255, 0.92)",
          300: "rgba(255, 255, 255, 0.80)",
          400: "rgba(255, 255, 255, 0.56)",
          500: "rgba(255, 255, 255, 0.50)",
          600: "rgba(255, 255, 255, 0.35)",
          700: "rgba(255, 255, 255, 0.16)",
          800: "rgba(255, 255, 255, 0.08)",
          900: "rgba(255, 255, 255, 0.04)",
          950: "rgba(255, 255, 255, 0)",
        },
        brand: {
          25: "#F8FAFF",
          50: "#EDF2FF",
          100: "#D6E2FD",
          200: "#B7CDFC",
          300: "#88AEFB",
          400: "#6895F8",
          500: "#2F61F3",
          600: "#1A45E6",
          700: "#1234D1",
          800: "#182CA3",
          900: "#1A297D",
          950: "#161C49",
        },
        error: {
          25: "#FFFBFA",
          50: "#FEF3F2",
          100: "#FEE4E2",
          200: "#FECDCA",
          300: "#FDA29B",
          400: "#F97066",
          500: "#F04438",
          600: "#D92D20",
          700: "#B42318",
          800: "#912018",
          900: "#7A271A",
          950: "#55160C",
        },
        warning: {
          25: "#FFFCF5",
          50: "#FFFAEB",
          100: "#FEF0C7",
          200: "#FEDF89",
          300: "#FEC84B",
          400: "#FDB022",
          500: "#F79009",
          600: "#DC6803",
          700: "#B54708",
          800: "#93370D",
          900: "#7A2E0E",
          950: "#4E1D09",
        },
        success: {
          25: "#F6FEF9",
          50: "#ECFDF3",
          100: "#DCFAE6",
          200: "#ABEFC6",
          300: "#75E0A7",
          400: "#47CD89",
          500: "#17B26A",
          600: "#079455",
          700: "#067647",
          800: "#085D3A",
          900: "#074D31",
          950: "#053321",
        },
        "gray-blue": {
          25: "#FCFCFD",
          50: "#F8F9FC",
          100: "#EAECF5",
          200: "#D5D9EB",
          300: "#B3B8DB",
          400: "#717BBC",
          500: "#4E5BA6",
          600: "#3E4784",
          700: "#363F72",
          800: "#293056",
          900: "#101323",
          950: "#0D0F1C",
        },
        "gray-cool": {
          25: "#FCFCFD",
          50: "#F9F9FB",
          100: "#EFF1F5",
          200: "#DCDFEA",
          300: "#B9C0D4",
          400: "#7D89B0",
          500: "#5D6B98",
          600: "#4A5578",
          700: "#404968",
          800: "#30374F",
          900: "#111322",
          950: "#0E101B",
        },
        "gray-modern": {
          25: "#FCFCFD",
          50: "#F8FAFC",
          100: "#EEF2F6",
          200: "#E3E8EF",
          300: "#CDD5DF",
          400: "#9AA4B2",
          500: "#697586",
          600: "#4B5565",
          700: "#364152",
          800: "#202939",
          900: "#121926",
          950: "#0D121C",
        },
        "gray-neutral": {
          25: "#FCFCFD",
          50: "#F9FAFB",
          100: "#F3F4F6",
          200: "#E5E7EB",
          300: "#D2D6DB",
          400: "#9DA4AE",
          500: "#6C737F",
          600: "#4D5761",
          700: "#384250",
          800: "#1F2A37",
          900: "#111927",
          950: "#0D121C",
        },
        "gray-iron": {
          25: "#FCFCFC",
          50: "#FAFAFA",
          100: "#F4F4F5",
          200: "#E4E4E7",
          300: "#D1D1D6",
          400: "#A0A0AB",
          500: "#70707B",
          600: "#51525C",
          700: "#3F3F46",
          800: "#26272B",
          900: "#1A1A1E",
          950: "#131316",
        },
        "gray-true": {
          25: "#FCFCFC",
          50: "#F7F7F7",
          100: "#F5F5F5",
          200: "#E5E5E5",
          300: "#D6D6D6",
          400: "#A3A3A3",
          500: "#737373",
          600: "#525252",
          700: "#424242",
          800: "#292929",
          900: "#141414",
          950: "#0F0F0F",
        },
        "gray-warm": {
          25: "#FDFDFC",
          50: "#FAFAF9",
          100: "#F5F5F4",
          200: "#E7E5E4",
          300: "#D7D3D0",
          400: "#A9A29D",
          500: "#79716B",
          600: "#57534E",
          700: "#44403C",
          800: "#292524",
          900: "#1C1917",
          950: "#171412",
        },
        moss: {
          25: "#FAFDF7",
          50: "#F5FBEE",
          100: "#E6F4D7",
          200: "#CEEAB0",
          300: "#ACDC79",
          400: "#86CB3C",
          500: "#669F2A",
          600: "#4F7A21",
          700: "#3F621A",
          800: "#335015",
          900: "#2B4212",
          950: "#1A280B",
        },
        "green-light": {
          25: "#FAFEF5",
          50: "#F3FEE7",
          100: "#E3FBCC",
          200: "#D0F8AB",
          300: "#A6EF67",
          400: "#85E13A",
          500: "#66C61C",
          600: "#4CA30D",
          700: "#3B7C0F",
          800: "#326212",
          900: "#2B5314",
          950: "#15290A",
        },
        green: {
          25: "#F6FEF9",
          50: "#EDFCF2",
          100: "#D3F8DF",
          200: "#AAF0C4",
          300: "#73E2A3",
          400: "#3CCB7F",
          500: "#16B364",
          600: "#099250",
          700: "#087443",
          800: "#095C37",
          900: "#084C2E",
          950: "#052E1C",
        },
        teal: {
          25: "#F6FEFC",
          50: "#F0FDF9",
          100: "#CCFBEF",
          200: "#99F6E0",
          300: "#5FE9D0",
          400: "#2ED3B7",
          500: "#15B79E",
          600: "#0E9384",
          700: "#107569",
          800: "#125D56",
          900: "#134E48",
          950: "#0A2926",
        },
        cyan: {
          25: "#F5FEFF",
          50: "#ECFDFF",
          100: "#CFF9FE",
          200: "#A5F0FC",
          300: "#67E3F9",
          400: "#22CCEE",
          500: "#06AED4",
          600: "#088AB2",
          700: "#0E7090",
          800: "#155B75",
          900: "#164C63",
          950: "#0D2D3A",
        },
        "blue-light": {
          25: "#F5FBFF",
          50: "#F0F9FF",
          100: "#E0F2FE",
          200: "#B9E6FE",
          300: "#7CD4FD",
          400: "#36BFFA",
          500: "#0BA5EC",
          600: "#0086C9",
          700: "#026AA2",
          800: "#065986",
          900: "#0B4A6F",
          950: "#062C41",
        },
        blue: {
          25: "#F5FAFF",
          50: "#EFF8FF",
          100: "#D1E9FF",
          200: "#B2DDFF",
          300: "#84CAFF",
          400: "#53B1FD",
          500: "#2E90FA",
          600: "#1570EF",
          700: "#175CD3",
          800: "#1849A9",
          900: "#194185",
          950: "#102A56",
        },
        "blue-dark": {
          25: "#F5F8FF",
          50: "#EFF4FF",
          100: "#D1E0FF",
          200: "#B2CCFF",
          300: "#84ADFF",
          400: "#528BFF",
          500: "#2970FF",
          600: "#155EEF",
          700: "#004EEB",
          800: "#0040C1",
          900: "#00359E",
          950: "#002266",
        },
        tangerine: {
          25: "#FFFAF9",
          50: "#FEF3F0",
          100: "#FEE5DD",
          200: "#FECEC2",
          300: "#FD9C84",
          400: "#FB8264",
          500: "#F15C3B",
          600: "#DB4424",
          700: "#B4361C",
          800: "#912F1A",
          900: "#752C1B",
          950: "#3E160D",
        },
        indigo: {
          25: "#F5F8FF",
          50: "#EEF4FF",
          100: "#E0EAFF",
          200: "#C7D7FE",
          300: "#A4BCFD",
          400: "#8098F9",
          500: "#6172F3",
          600: "#444CE7",
          700: "#3538CD",
          800: "#2D31A6",
          900: "#2D3282",
          950: "#1F235B",
        },
        violet: {
          25: "#FBFAFF",
          50: "#F5F3FF",
          100: "#ECE9FE",
          200: "#DDD6FE",
          300: "#C3B5FD",
          400: "#A48AFB",
          500: "#875BF7",
          600: "#7839EE",
          700: "#6927DA",
          800: "#5720B7",
          900: "#491C96",
          950: "#2E125E",
        },
        purple: {
          25: "#FAFAFF",
          50: "#F4F3FF",
          100: "#EBE9FE",
          200: "#D9D6FE",
          300: "#BDB4FE",
          400: "#9B8AFB",
          500: "#7A5AF8",
          600: "#6938EF",
          700: "#5925DC",
          800: "#4A1FB8",
          900: "#3E1C96",
          950: "#27115F",
        },
        pink: {
          25: "#FEF6FB",
          50: "#FDF2FA",
          100: "#FCE7F6",
          200: "#FCCEEE",
          300: "#FAA7E0",
          400: "#F670C7",
          500: "#EE46BC",
          600: "#DD2590",
          700: "#C11574",
          800: "#9E165F",
          900: "#851651",
          950: "#4E0D30",
        },
        rose: {
          25: "#FFF5F6",
          50: "#FFF1F3",
          100: "#FFE4E8",
          200: "#FECDD6",
          300: "#FEA3B4",
          400: "#FD6F8E",
          500: "#F63D68",
          600: "#E31B54",
          700: "#C01048",
          800: "#A11043",
          900: "#89123E",
          950: "#510B24",
        },
        "orange-dark": {
          25: "#FFF9F5",
          50: "#FFF4ED",
          100: "#FFE6D5",
          200: "#FFD6AE",
          300: "#FF9C66",
          400: "#FF692E",
          500: "#FF4405",
          600: "#E62E05",
          700: "#BC1B06",
          800: "#97180C",
          900: "#771A0D",
          950: "#57130A",
        },
        orange: {
          25: "#FEFAF5",
          50: "#FEF6EE",
          100: "#FDEAD7",
          200: "#F9DBAF",
          300: "#F7B27A",
          400: "#F38744",
          500: "#EF6820",
          600: "#E04F16",
          700: "#B93815",
          800: "#932F19",
          900: "#772917",
          950: "#511C10",
        },
        yellow: {
          25: "#FEFDF0",
          50: "#FEFBE8",
          100: "#FEF7C3",
          200: "#FEEE95",
          300: "#FDE272",
          400: "#FAC515",
          500: "#EAAA08",
          600: "#CA8504",
          700: "#A15C07",
          800: "#854A0E",
          900: "#713B12",
          950: "#542C0D",
        },
        /* ── Foreground (icon) tokens ─ usage: text-fg-primary, bg-fg-error, etc. ── */
        fg: {
          primary: "var(--color-fg-primary)",
          secondary: "var(--color-fg-secondary)",
          tertiary: "var(--color-fg-tertiary)",
          quaternary: "var(--color-fg-quaternary)",
          white: "var(--color-fg-white)",
          disabled: "var(--color-fg-disabled)",
          "brand-primary": "var(--color-fg-brand-primary)",
          "brand-secondary": "var(--color-fg-brand-secondary)",
          error: "var(--color-fg-error)",
          warning: "var(--color-fg-warning)",
          success: "var(--color-fg-success)",
        },
        /* ── Focus tokens ── */
        focus: {
          ring: "var(--color-focus-ring)",
          "ring-error": "var(--color-focus-ring-error)",
        },
        /* ── Component color tokens ── */
        alpha: {
          "white-10": "rgba(255, 255, 255, 0.1)",
          "white-20": "rgba(255, 255, 255, 0.2)",
          "white-30": "rgba(255, 255, 255, 0.3)",
          "white-40": "rgba(255, 255, 255, 0.4)",
          "white-50": "rgba(255, 255, 255, 0.5)",
          "white-60": "rgba(255, 255, 255, 0.6)",
          "white-70": "rgba(255, 255, 255, 0.7)",
          "white-80": "rgba(255, 255, 255, 0.8)",
          "white-90": "rgba(255, 255, 255, 0.9)",
          "white-100": "#FFFFFF",
          "black-10": "rgba(0, 0, 0, 0.1)",
          "black-20": "rgba(0, 0, 0, 0.2)",
          "black-30": "rgba(0, 0, 0, 0.3)",
          "black-40": "rgba(0, 0, 0, 0.4)",
          "black-50": "rgba(0, 0, 0, 0.5)",
          "black-60": "rgba(0, 0, 0, 0.6)",
          "black-70": "rgba(0, 0, 0, 0.7)",
          "black-80": "rgba(0, 0, 0, 0.8)",
          "black-90": "rgba(0, 0, 0, 0.9)",
          "black-100": "#000000",
        },
        /* ── Utility color tokens (component-level subsets 50–700/900) usage: bg-utility-gray-200, border-utility-gray-200, etc. ── */
        "utility-gray": {
          50: "#F8F9FC",
          100: "#EAECF5",
          200: "#D5D9EB",
          300: "#B3B8DB",
          400: "#717BBC",
          500: "#4E5BA6",
          600: "#3E4784",
          700: "#363F72",
          800: "#293056",
          900: "#101323",
        },
        "utility-brand": {
          50: "#EDF2FF",
          100: "#D6E2FD",
          200: "#B7CDFC",
          300: "#88AEFB",
          400: "#6895F8",
          500: "#2F61F3",
          600: "#1A45E6",
          700: "#1234D1",
          800: "#182CA3",
          900: "#1A297D",
        },
        "utility-error": {
          50: "#FEF3F2",
          100: "#FEE4E2",
          200: "#FECDCA",
          300: "#FDA29B",
          400: "#F97066",
          500: "#F04438",
          600: "#D92D20",
          700: "#B42318",
        },
        "utility-warning": {
          50: "#FFFAEB",
          100: "#FEF0C7",
          200: "#FEDF89",
          300: "#FEC84B",
          400: "#FDB022",
          500: "#F79009",
          600: "#DC6803",
          700: "#B54708",
        },
        "utility-success": {
          50: "#ECFDF3",
          100: "#DCFAE6",
          200: "#ABEFC6",
          300: "#75E0A7",
          400: "#47CD89",
          500: "#17B26A",
          600: "#079455",
          700: "#067647",
        },
        "utility-gray-blue": {
          50: "#F8F9FC",
          100: "#EAECF5",
          200: "#D5D9EB",
          300: "#B3B8DB",
          400: "#717BBC",
          500: "#4E5BA6",
          600: "#3E4784",
          700: "#363F72",
        },
        "utility-green": {
          50: "#EDFCF2",
          100: "#D3F8DF",
          200: "#AAF0C4",
          300: "#73E2A3",
          400: "#3CCB7F",
          500: "#16B364",
          600: "#099250",
          700: "#087443",
        },
        "utility-blue-light": {
          50: "#F0F9FF",
          100: "#E0F2FE",
          200: "#B9E6FE",
          300: "#7CD4FD",
          400: "#36BFFA",
          500: "#0BA5EC",
          600: "#0086C9",
          700: "#026AA2",
        },
        "utility-blue": {
          50: "#EFF8FF",
          100: "#D1E9FF",
          200: "#B2DDFF",
          300: "#84CAFF",
          400: "#53B1FD",
          500: "#2E90FA",
          600: "#1570EF",
          700: "#175CD3",
        },
        "utility-blue-dark": {
          50: "#EFF4FF",
          100: "#D1E0FF",
          200: "#B2CCFF",
          300: "#84ADFF",
          400: "#528BFF",
          500: "#2970FF",
          600: "#155EEF",
          700: "#004EEB",
        },
        "utility-indigo": {
          50: "#EEF4FF",
          100: "#E0EAFF",
          200: "#C7D7FE",
          300: "#A4BCFD",
          400: "#8098F9",
          500: "#6172F3",
          600: "#444CE7",
          700: "#3538CD",
        },
        "utility-purple": {
          50: "#F4F3FF",
          100: "#EBE9FE",
          200: "#D9D6FE",
          300: "#BDB4FE",
          400: "#9B8AFB",
          500: "#7A5AF8",
          600: "#6938EF",
          700: "#5925DC",
        },
        "utility-fuchsia": {
          50: "#FEF3F0",
          100: "#FEE5DD",
          200: "#FECEC2",
          300: "#FD9C84",
          400: "#FB8264",
          500: "#F15C3B",
          600: "#DB4424",
          700: "#B4361C",
        },
        "utility-pink": {
          50: "#FDF2FA",
          100: "#FCE7F6",
          200: "#FCCEEE",
          300: "#FAA7E0",
          400: "#F670C7",
          500: "#EE46BC",
          600: "#DD2590",
          700: "#C11574",
        },
        "utility-orange-dark": {
          50: "#FFF4ED",
          100: "#FFE6D5",
          200: "#FFD6AE",
          300: "#FF9C66",
          400: "#FF692E",
          500: "#FF4405",
          600: "#E62E05",
          700: "#BC1B06",
        },
        "utility-orange": {
          50: "#FEF6EE",
          100: "#FDEAD7",
          200: "#F9DBAF",
          300: "#F7B27A",
          400: "#F38744",
          500: "#EF6820",
          600: "#E04F16",
          700: "#B93815",
        },
        "utility-yellow": {
          50: "#FEFBE8",
          100: "#FEF7C3",
          200: "#FEEE95",
          300: "#FDE272",
          400: "#FAC515",
          500: "#EAAA08",
          600: "#CA8504",
          700: "#A15C07",
        },
        /* ── Button component tokens ── */
        button: {
          "primary-icon": "#F8FAFF",
          "primary-icon-hover": "#B7CDFC",
          "destructive-primary-icon": "#FDA29B",
          "destructive-primary-icon-hover": "#FECDCA",
        },
        /* ── Icon component tokens ── */
        "icon-fg": {
          brand: "#1A45E6",
          "brand-on-brand": "#B7CDFC",
        },
      },
      /* ── Text color tokens ─ text-primary, text-secondary, text-brand-primary ── */
      textColor: {
        primary: "var(--color-text-primary)",
        "primary-on-brand": "var(--color-text-primary-on-brand)",
        secondary: "var(--color-text-secondary)",
        "secondary-on-brand": "var(--color-text-secondary-on-brand)",
        tertiary: "var(--color-text-tertiary)",
        "tertiary-on-brand": "var(--color-text-tertiary-on-brand)",
        quaternary: "var(--color-text-quaternary)",
        "quaternary-on-brand": "var(--color-text-quaternary-on-brand)",
        white: "var(--color-text-white)",
        "white-subtle": "var(--color-text-white-subtle)",
        disabled: "var(--color-text-disabled)",
        placeholder: "var(--color-text-placeholder)",
        "placeholder-subtle": "var(--color-text-placeholder-subtle)",
        "brand-primary": "var(--color-text-brand-primary)",
        "brand-secondary": "var(--color-text-brand-secondary)",
        "brand-tertiary": "var(--color-text-brand-tertiary)",
        "brand-quaternary": "var(--color-text-brand-quaternary)",
        error: "var(--color-text-error)",
        warning: "var(--color-text-warning)",
        success: "var(--color-text-success)",
        fg: {
          primary: "var(--color-fg-primary)",
          secondary: "var(--color-fg-secondary)",
          tertiary: "var(--color-fg-tertiary)",
          quaternary: "var(--color-fg-quaternary)",
          white: "var(--color-fg-white)",
          disabled: "var(--color-fg-disabled)",
          "brand-primary": "var(--color-fg-brand-primary)",
          "brand-secondary": "var(--color-fg-brand-secondary)",
          error: "var(--color-fg-error)",
          warning: "var(--color-fg-warning)",
          success: "var(--color-fg-success)",
        },
      },
      /* ── Background color tokens ─ bg-primary, bg-brand-solid, etc. ── */
      backgroundColor: {
        primary: "var(--color-bg-primary)",
        "primary-solid": "var(--color-bg-primary-solid)",
        secondary: "var(--color-bg-secondary)",
        "secondary-solid": "var(--color-bg-secondary-solid)",
        tertiary: "var(--color-bg-tertiary)",
        quaternary: "var(--color-bg-quaternary)",
        disabled: "var(--color-bg-disabled)",
        "disabled-subtle": "var(--color-bg-disabled-subtle)",
        "brand-primary": "var(--color-bg-brand-primary)",
        "brand-primary-subtle": "var(--color-bg-brand-primary-subtle)",
        "brand-secondary": "var(--color-bg-brand-secondary)",
        "brand-solid": "var(--color-bg-brand-solid)",
        "brand-section": "var(--color-bg-brand-section)",
        error: "var(--color-bg-error)",
        warning: "var(--color-bg-warning)",
        success: "var(--color-bg-success)",
        fg: {
          "brand-primary": "var(--color-fg-brand-primary)",
          "brand-secondary": "var(--color-fg-brand-secondary)",
          error: "var(--color-fg-error)",
          warning: "var(--color-fg-warning)",
          success: "var(--color-fg-success)",
        },
      },
      /* ── Border color tokens ─ border-primary, border-brand, etc. ── */
      borderColor: {
        primary: "var(--color-border-primary)",
        secondary: "var(--color-border-secondary)",
        tertiary: "var(--color-border-tertiary)",
        disabled: "var(--color-border-disabled)",
        "disabled-subtle": "var(--color-border-disabled-subtle)",
        brand: "var(--color-border-brand)",
        error: "var(--color-border-error)",
      },
      fontFamily: {
        display: ["Literata", "Georgia", "serif"],
        body: ["Plus Jakarta Sans", "system-ui", "sans-serif"],
        number: ["Iceland", "system-ui", "sans-serif"],
        sans: ["Plus Jakarta Sans", "system-ui", "sans-serif"],
        mono: ["JetBrains Mono", "monospace"],
      },
      fontSize: {
        /* Semantic scale — rem values based on 16px root (Figma design tokens) */
        tiny: ["0.6875rem", { lineHeight: "1rem" }],
        caption: ["0.8125rem", { lineHeight: "1rem" }],
        "body-small": ["0.9375rem", { lineHeight: "1.25rem" }],
        body: ["1.0625rem", { lineHeight: "1.5rem" }],
        h3: ["1.1875rem", { lineHeight: "1.5rem" }],
        h2: ["1.3125rem", { lineHeight: "1.75rem" }],
        h1: ["1.5rem", { lineHeight: "2rem" }],
        display: ["2.125rem", { lineHeight: "2.75rem" }],
        "display-xl": ["4.5rem", { lineHeight: "5rem" }],
        /* Standard aliases for general use */
        xs: ["0.8125rem", { lineHeight: "1rem" }],
        sm: ["0.9375rem", { lineHeight: "1.25rem" }],
        base: ["1.0625rem", { lineHeight: "1.5rem" }],
        lg: ["1.1875rem", { lineHeight: "1.5rem" }],
        xl: ["1.3125rem", { lineHeight: "1.75rem" }],
        "2xl": ["1.5rem", { lineHeight: "2rem" }],
        "3xl": ["2.125rem", { lineHeight: "2.75rem" }],
      },
      letterSpacing: {
        tighter: "-0.02em",
      },
      width: {
        xxs: "320px",
        xs: "384px",
        sm: "480px",
        md: "560px",
        lg: "640px",
        xl: "768px",
        "2xl": "1024px",
        "3xl": "1280px",
        "4xl": "1440px",
        "5xl": "1600px",
        "6xl": "1920px",
        "paragraph-max": "720px",
      },
      maxWidth: {
        xxs: "320px",
        xs: "384px",
        sm: "480px",
        md: "560px",
        lg: "640px",
        xl: "768px",
        "2xl": "1024px",
        "3xl": "1280px",
        "4xl": "1440px",
        "5xl": "1600px",
        "6xl": "1920px",
        "paragraph-max": "720px",
        "container-max-width-desktop": "1280px",
      },
      boxShadow: {
        xs: "var(--shadow-xs)",
        sm: "var(--shadow-sm)",
        md: "var(--shadow-md)",
        lg: "var(--shadow-lg)",
        xl: "var(--shadow-xl)",
        "2xl": "var(--shadow-2xl)",
        "3xl": "var(--shadow-3xl)",
      },
      backgroundImage: {
        /* Gray gradients */
        "gradient-gray-600-500-90": "var(--gradient-gray-600-500-90)",
        "gradient-gray-700-600-45": "var(--gradient-gray-700-600-45)",
        "gradient-gray-800-600-45": "var(--gradient-gray-800-600-45)",
        "gradient-gray-800-600-90": "var(--gradient-gray-800-600-90)",
        "gradient-gray-800-700-26": "var(--gradient-gray-800-700-26)",
        "gradient-gray-900-600-45": "var(--gradient-gray-900-600-45)",
        "gradient-gray-900-700-45": "var(--gradient-gray-900-700-45)",
        "gradient-gray-50-white-180": "var(--gradient-gray-50-white-180)",
        "gradient-gray-100-white-180": "var(--gradient-gray-100-white-180)",
        "gradient-gray-100-25-180": "var(--gradient-gray-100-25-180)",
        "gradient-gray-100-50-180": "var(--gradient-gray-100-50-180)",
        "gradient-gray-200-25-180": "var(--gradient-gray-200-25-180)",
        "gradient-gray-200-50-180": "var(--gradient-gray-200-50-180)",
        "gradient-gray-200-100-180": "var(--gradient-gray-200-100-180)",
        /* Brand gradients */
        "gradient-brand-600-500-90": "var(--gradient-brand-600-500-90)",
        "gradient-brand-700-600-45": "var(--gradient-brand-700-600-45)",
        "gradient-brand-800-600-45": "var(--gradient-brand-800-600-45)",
        "gradient-brand-800-600-90": "var(--gradient-brand-800-600-90)",
        "gradient-brand-800-700-26": "var(--gradient-brand-800-700-26)",
        "gradient-brand-900-600-45": "var(--gradient-brand-900-600-45)",
        "gradient-brand-900-700-45": "var(--gradient-brand-900-700-45)",
        /* Linear (decorative) gradients */
        ...Object.fromEntries(
          Array.from({ length: 91 }, (_, i) => {
            const num = String(i + 1).padStart(2, "0")
            return [`gradient-linear-${num}`, `var(--gradient-linear-${num})`]
          }),
        ),
      },
    },
  },
  plugins: [],
}
